...
class MultiSrcAttention(torch.nn.Module):
    def __init__(self, dropout=0.1, n_embed=1024, num_heads=8):
        ...
        self.c_attn_2 = torch.nn.Linear(n_embed, 3 * n_embed) # QKV
        self.c_proj_2 = torch.nn.Linear(n_embed, n_embed)

        # regularization
        self.attn_dropout_1 = torch.nn.Dropout(dropout)
        self.resid_dropout_1 = torch.nn.Dropout(dropout)
        self.attn_dropout_2 = torch.nn.Dropout(dropout)
        self.resid_dropout_2 = torch.nn.Dropout(dropout)
    ...
    def forward(self, x, y):
        ...
        q_1, k_1, v_1  = self.c_attn_1(x).split(self.embed_dim, dim=2)
        q_2, k_2, v_2  = self.c_attn_2(y).split(self.embed_dim, dim=2)


        k_1 = k_1.view(B, M, self.num_heads, C // self.num_heads).transpose(1, 2) # (B, nh, M, hs)
        q_1 = q_1.view(B, M, self.num_heads, C // self.num_heads).transpose(1, 2) # (B, nh, M, hs)
        v_1 = v_1.view(B, M, self.num_heads, C // self.num_heads).transpose(1, 2) # (B, nh, M, hs)

        k_2 = k_2.view(B, N, self.num_heads, C // self.num_heads).transpose(1, 2) # (B, nh, N, hs)
        q_2 = q_2.view(B, N, self.num_heads, C // self.num_heads).transpose(1, 2) # (B, nh, N, hs)
        v_2 = v_2.view(B, N, self.num_heads, C // self.num_heads).transpose(1, 2) # (B, nh, N, hs)
        
        # causal self-attention; Self-attend: (B, nh, M, hs) x (B, nh, hs, N) -> (B, nh, M, N)
        # causal self-attention; Self-attend: (B, nh, N, hs) x (B, nh, hs, M) -> (B, nh, N, M)
        if self.flash:
            # efficient attention using Flash Attention CUDA kernels
            out_1 = torch.nn.functional.scaled_dot_product_attention(q_1, k_2, v_2, attn_mask=None, dropout_p=self.dropout if self.training else 0, is_causal=False)
            out_2 = torch.nn.functional.scaled_dot_product_attention(q_2, k_1, v_1, attn_mask=None, dropout_p=self.dropout if self.training else 0, is_causal=False)
        else:
            # manual implementation of attention
            att_1 = (q_1 @ k_2.transpose(-2, -1)) * (1.0 / math.sqrt(k_2.size(-1)))
            att_1 = F.softmax(att_1, dim=-1)
            att_1 = self.attn_dropout_1(att_1)
            out_1 = att_1 @ v_2 # (B, nh, M, N) x (B, nh, N, hs) -> (B, nh, M, hs)

            att_2 = (q_2 @ k_1.transpose(-2, -1)) * (1.0 / math.sqrt(k_1.size(-1)))
            att_2 = F.softmax(att_2, dim=-1)
            att_2 = self.attn_dropout_2(att_2)
            out_2 = att_2 @ v_1 # (B, nh, N, M) x (B, nh, M, hs) -> (B, nh, N, hs)

        out_1 = out_1.transpose(1, 2).contiguous().view(B, M, C) # re-assemble all head outputs side by side
        out_2 = out_2.transpose(1, 2).contiguous().view(B, N, C) # re-assemble all head outputs side by side

        # output projection
        out_1 = self.resid_dropout_1(self.c_proj_1(out_1))
        out_2 = self.resid_dropout_2(self.c_proj_2(out_2))
        return out_1,out_2
...
class DualEncoderDualDecoder(PreTrainedModel):
    ...
    def forward(self,
        ...
        concatenated_hidden_states = torch.cat([enc_out_1.last_hidden_state, enc_out_2.last_hidden_state,enc_out_1_last_hidden_state,enc_out_2_last_hidden_state],dim=1)
