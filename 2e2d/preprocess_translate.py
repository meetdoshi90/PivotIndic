INDIC_NLP_LIB_HOME = "../IndicTrans2-main/indic_nlp_library"
INDIC_NLP_RESOURCES = "../IndicTrans2-main/indic_nlp_resources"
import sys
...
def preprocess_list(
    ...
    n = 0
    num_lines = len(inp_list)

    if iso_lang == "en":
        out_lines = Parallel(n_jobs=-1, backend="multiprocessing")(
            delayed(preprocess_line)(line, None, lang, transliterate, remove_tag) for line in tqdm(inp_list, total=num_lines)
        )
        n = len(out_lines)
    else:
        normfactory = indic_normalize.IndicNormalizerFactory()
        ...
        out_lines = Parallel(n_jobs=-1, backend="multiprocessing")(
            delayed(preprocess_line)(line, normalizer, lang, transliterate, remove_tag)
            for line in tqdm(inp_list, total=num_lines)
        )
...
def caller(
    ...
    if remove_tag.lower() == "true":
        ...
        remove_tag = False
    return preprocess_list(inp_list, lang, transliterate, remove_tag)
