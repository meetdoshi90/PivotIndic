...
def get_datasets(src1_file, src2_file, src3_file, src4_file, tgt_file,type_data='train'):
    file = open(src1_file)
    ...
    file = open(src3_file)
    src3_lines = file.readlines()
    file.close()
    
    file = open(src4_file)
    src4_lines = file.readlines()
    file.close()
    ...
    src3_lines = [line.strip().replace("\n", "") for line in src3_lines]
    src4_lines = [line.strip().replace("\n", "") for line in src4_lines]
    tgt_lines = [line.strip().replace("\n", "") for line in tgt_lines]
    train = [type_data]*len(src1_lines)
    
    train_dataset = Dataset.from_dict({"src1": src1_lines, "src2":src2_lines,"src3":src3_lines,"src3":src3_lines, "src4":src4_lines, "tgt":tgt_lines, "type": train})
...
class DataCollatorForSeq2SeqTraining(DataCollator):
    ...
    def __call__(self, examples) -> Dict[str, torch.Tensor]:
        ...
        batch_indic_inp_1 = [example['inputs_2'] for example in examples]
        batch_indic_inp_2 = [example['inputs_3'] for example in examples]
        batch_indic_inp_3 = [example['inputs_4'] for example in examples]
        ...
        pivot_out_1 = indic_indic_tokenizer(
            batch_indic_inp_1,
            src=False,
            truncation=True,
            padding="longest",
        ...
        inputs_3 = indic_indic_tokenizer(
            batch_indic_inp_2,
            src=True,
            truncation=True,
            padding="longest",
        ...
        pivot_out_2 = indic_indic_tokenizer(
            batch_indic_inp_2,
            src=False,
            truncation=True,
            padding="longest",
        ...
        inputs_4 = indic_indic_tokenizer(
            batch_indic_inp_3,
            src=True,
            truncation=True,
            padding="longest",
        ...
        pivot_out_3 = indic_indic_tokenizer(
            batch_indic_inp_3,
            src=False,
            truncation=True,
            padding="longest",
        ...
        shape3 = inputs_3.input_ids.shape[-1]
        shape4 = inputs_4.input_ids.shape[-1]
        inputs = {
            'input_ids': torch.cat((inputs_1.input_ids,inputs_2.input_ids,inputs_3.input_ids,inputs_4.input_ids),dim=1),
            'attention_mask': torch.cat((inputs_1.attention_mask,inputs_2.attention_mask,inputs_3.attention_mask,inputs_4.attention_mask),dim=1),
        }
        ...
        pivot_out_1.input_ids = torch.cat((torch.ones((pivot_out_1.input_ids.shape[0],1),dtype=torch.long)*2, pivot_out_1.input_ids),dim=-1)
        pivot_out_1.attention_mask = torch.cat((torch.ones((pivot_out_1.attention_mask.shape[0],1),dtype=torch.long), pivot_out_1.attention_mask),dim=-1)

        pivot_out_2.input_ids = torch.cat((torch.ones((pivot_out_2.input_ids.shape[0],1),dtype=torch.long)*2, pivot_out_2.input_ids),dim=-1)
        pivot_out_2.attention_mask = torch.cat((torch.ones((pivot_out_2.attention_mask.shape[0],1),dtype=torch.long), pivot_out_2.attention_mask),dim=-1)


        pivot_out_3.input_ids = torch.cat((torch.ones((pivot_out_3.input_ids.shape[0],1),dtype=torch.long)*2, pivot_out_3.input_ids),dim=-1)
        pivot_out_3.attention_mask = torch.cat((torch.ones((pivot_out_3.attention_mask.shape[0],1),dtype=torch.long), pivot_out_3.attention_mask),dim=-1)
        ...
        return {
            ...
            'pivot_1_decoder_input_ids': pivot_out_1['input_ids'],
            'pivot_2_decoder_input_ids': pivot_out_2['input_ids'],
            'pivot_3_decoder_input_ids': pivot_out_3['input_ids'],
...
class DualEncoderDualDecoder(PreTrainedModel):
    def __init__(self,model_names,alpha=0.5,beta=0.5,PAD_ID=1,config=None,genconfig=None):
        ...
        self.scorer_model = self.initialize_model(model_names[0], "en-indic", "")
        self.scorer_model.eval() #used to weigh the pivot instance P(Z|X)
    ...
    def forward(self,
            ...
            pivot_1_decoder_input_ids: Optional[torch.LongTensor] = None,
            pivot_2_decoder_input_ids: Optional[torch.LongTensor] = None,
            pivot_3_decoder_input_ids: Optional[torch.LongTensor] = None,
            decoder_attention_mask: Optional[torch.LongTensor] = None,
            labels: Optional[torch.LongTensor] = None,
            shape1: Optional[int] = None,
            shape2: Optional[int] = None,
            shape3: Optional[int] = None,
            shape4: Optional[int] = None,
        ...
        inputs_2 = {
            'input_ids': input_ids[:,shape1:shape1+shape2],
            'attention_mask': attention_mask[:,shape1:shape1+shape2],
        }
        inputs_3 = {
            'input_ids': input_ids[:,shape1+shape2:shape1+shape2+shape3],
            'attention_mask': attention_mask[:,shape1+shape2:shape1+shape2+shape3],
        }
        inputs_4 = {
            'input_ids': input_ids[:,shape1+shape2+shape3:],
            'attention_mask': attention_mask[:,shape1+shape2+shape3:],
        }
        inputs_1 = BatchEncoding(inputs_1,tensor_type='pt')
        inputs_2 = BatchEncoding(inputs_2,tensor_type='pt')
        inputs_3 = BatchEncoding(inputs_3,tensor_type='pt')
        inputs_4 = BatchEncoding(inputs_4,tensor_type='pt')
        ...
        enc_out_3 = self.indic_indic_model.model.encoder(
            **inputs_3,
            return_dict=True
        )

        enc_out_4 = self.indic_indic_model.model.encoder(
            **inputs_4,
            return_dict=True
        )
        ...
        concatenated_hidden_states_1 = torch.cat([enc_out_1.last_hidden_state,enc_out_2.last_hidden_state],dim=1)
        concatenated_hidden_states_2 = torch.cat([enc_out_1.last_hidden_state,enc_out_3.last_hidden_state],dim=1)
        concatenated_hidden_states_3 = torch.cat([enc_out_1.last_hidden_state,enc_out_4.last_hidden_state],dim=1)
        #concatenated_hidden_states = enc_out_1.last_hidden_state
        #print('concat enc states',concatenated_hidden_states.shape)
        dec_out_1 = self.en_indic_model.model.decoder(
            input_ids=decoder_input_ids,
            encoder_hidden_states = concatenated_hidden_states_1,
            return_dict=True
        )
        dec_logits_1 = self.en_indic_model.lm_head(dec_out_1.last_hidden_state)

        dec_out_2 = self.en_indic_model.model.decoder(
            input_ids=decoder_input_ids,
            encoder_hidden_states = concatenated_hidden_states_2,
            return_dict=True
        )
        dec_logits_2 = self.en_indic_model.lm_head(dec_out_2.last_hidden_state)

        dec_out_3 = self.en_indic_model.model.decoder(
            input_ids=decoder_input_ids,
            encoder_hidden_states = concatenated_hidden_states_3,
            return_dict=True
        )
        dec_logits_3 = self.en_indic_model.lm_head(dec_out_3.last_hidden_state)
        ...
        p_y_given_xz_1 = F.softmax(dec_logits_1,dim=-1)
        p_y_given_xz_2 = F.softmax(dec_logits_2,dim=-1)
        p_y_given_xz_3 = F.softmax(dec_logits_3,dim=-1)

        if eval==False:
            with torch.no_grad():
                scorer_model_enc_out = self.scorer_model.model.encoder(
                    **inputs_1,
                    return_dict=True
                ).last_hidden_state

                scorer_model_dec_out_1 = self.scorer_model.model.decoder(
                    input_ids = pivot_1_decoder_input_ids[...,:-1],
                    encoder_hidden_states = scorer_model_enc_out,
                    return_dict=True
                ).last_hidden_state
                scorer_model_dec_logits_1 = self.scorer_model.lm_head(scorer_model_dec_out_1)
                scorer_normalized_out_1 = F.softmax(scorer_model_dec_logits_1,dim=-1)
                scorer_loss_1 = self.get_loss(scorer_normalized_out_1,pivot_1_decoder_input_ids[...,1:])

                scorer_model_dec_out_2 = self.scorer_model.model.decoder(
                    input_ids = pivot_2_decoder_input_ids[...,:-1],
                    encoder_hidden_states = scorer_model_enc_out,
                    return_dict=True
                ).last_hidden_state
                scorer_model_dec_logits_2 = self.scorer_model.lm_head(scorer_model_dec_out_2)
                scorer_normalized_out_2 = F.softmax(scorer_model_dec_logits_2,dim=-1)
                scorer_loss_2 = self.get_loss(scorer_normalized_out_2,pivot_2_decoder_input_ids[...,1:])

                scorer_model_dec_out_3 = self.scorer_model.model.decoder(
                    input_ids = pivot_3_decoder_input_ids[...,:-1],
                    encoder_hidden_states = scorer_model_enc_out,
                    return_dict=True
                ).last_hidden_state
                scorer_model_dec_logits_3 = self.scorer_model.lm_head(scorer_model_dec_out_3)
                scorer_normalized_out_3 = F.softmax(scorer_model_dec_logits_3,dim=-1)
                scorer_loss_3 = self.get_loss(scorer_normalized_out_3,pivot_3_decoder_input_ids[...,1:])

                p_z_given_x = F.softmax(torch.tensor([1.0/scorer_loss_1, 1.0/scorer_loss_2, 1.0/scorer_loss_3])).to('cuda')
        else:
            p_z_given_x = [1.0, 1.0, 1.0]


        #P(Y | X)=sum all possible Z { P(Y | X,Z)⋅P(Z | X) } 
        if eval==False:
            normalized_out = (p_y_given_xz_1*p_z_given_x[0] + p_y_given_xz_2*p_z_given_x[1] + p_y_given_xz_3*p_z_given_x[2])/3.0
        else:
            normalized_out = (p_y_given_xz_1 + p_y_given_xz_2 + p_y_given_xz_3)/3.0
    ...
    def prepare_inputs_for_generation(  
        ...
        if input_ids.shape[1]>(shape1+shape2):
            ...
            decoder_input_ids = torch.cat((decoder_input_ids,input_ids[:,shape1+shape2+shape3+shape4:]),dim=1)
            input_ids = input_ids[:,:shape1+shape2+shape3+shape4]
            attention_mask = attention_mask[:,:shape1+shape2+shape3+shape4]

        if decoder_input_ids is None:
            decoder_input_ids = torch.ones((input_ids.shape[0],1)).to(torch.long).to(device=device) * 2
        if decoder_attention_mask is None:
            decoder_attention_mask = torch.ones_like(decoder_input_ids).to(dtype=attention_mask.dtype).to(device=device)
        if pivot_1_decoder_input_ids is None:
            pivot_1_decoder_input_ids = torch.ones((input_ids.shape[0],1)).to(torch.long).to(device=device) * 2
        if pivot_2_decoder_input_ids is None:
            pivot_2_decoder_input_ids = torch.ones((input_ids.shape[0],1)).to(torch.long).to(device=device) * 2
        if pivot_3_decoder_input_ids is None:
            pivot_3_decoder_input_ids = torch.ones((input_ids.shape[0],1)).to(torch.long).to(device=device) * 2
        return {
            "input_ids": input_ids,  # encoder_outputs is defined. input_ids not needed
            "encoder_outputs": encoder_outputs,
            "decoder_input_ids": decoder_input_ids,
            "pivot_1_decoder_input_ids": pivot_1_decoder_input_ids,
            "pivot_2_decoder_input_ids": pivot_2_decoder_input_ids,
            "pivot_3_decoder_input_ids": pivot_3_decoder_input_ids,
...
class DataTrainingArguments:
    ...
    src_2_path: Optional[str] = field(
        default=None,
        metadata={"help": "The src input 2 training data file (a text file)."},
    )

    src_3_path: Optional[str] = field(
        default=None,
        metadata={"help": "The src input 3 training data file (a text file)."},
    )

    src_4_path: Optional[str] = field(
        default=None,
        metadata={"help": "The src input 4 training data file (a text file)."},
    )
    ...
    test_src_2_path: Optional[str] = field(
        default=None,
        metadata={"help": "The src input 2 test data file (a text file)."},
    )

    test_src_3_path: Optional[str] = field(
        default=None,
        metadata={"help": "The src input 3 test data file (a text file)."},
    )

    test_src_4_path: Optional[str] = field(
        default=None,
        metadata={"help": "The src input 4 test data file (a text file)."},
    )
...
if __name__ == "__main__":
    ...
    file_path3 = data_args.src_3_path
    src_lang_3 = file_path3.split('.')[-1]
    file_path4 = data_args.src_4_path
    src_lang_4 = file_path4.split('.')[-1]
    file_path5 = data_args.tgt_path
    tgt_lang = file_path5.split('.')[-1]


    test_file_path1 = data_args.test_src_1_path
    test_file_path2 = data_args.test_src_2_path
    test_file_path3 = data_args.test_src_3_path
    test_file_path4 = data_args.test_src_4_path
    test_file_path5 = data_args.test_tgt_path

    def preprocess_function(example,src_lang_1=src_lang_1,src_lang_2=src_lang_2,src_lang_3=src_lang_3,src_lang_4=src_lang_4,tgt_lang=tgt_lang):
        en_inp = example['src1']
        indic_inp_1 = example['src2']
        indic_inp_2 = example['src3']
        indic_inp_3 = example['src4']
        ...
        batch_indic_inp_1 = ip.preprocess_batch([indic_inp_1], src_lang=src_lang_2, tgt_lang=tgt_lang)
        batch_indic_inp_2 = ip.preprocess_batch([indic_inp_2], src_lang=src_lang_3, tgt_lang=tgt_lang)
        batch_indic_inp_3 = ip.preprocess_batch([indic_inp_3], src_lang=src_lang_4, tgt_lang=tgt_lang)
        ...
        return {"inputs_1": batch_en_inp[0], "inputs_2": batch_indic_inp_1[0], "inputs_3": batch_indic_inp_2[0], "inputs_4": batch_indic_inp_3[0], "outputs":batch_indic_target[0], "type": type_data}


    train_dataset = get_datasets(file_path1,file_path2,file_path3,file_path4,file_path5)
    train_dataset = train_dataset.shuffle(seed=training_args.seed).map(preprocess_function, num_proc=16)


    test_dataset = get_datasets(test_file_path1,test_file_path2,test_file_path3, test_file_path4, test_file_path5)
    ...
    with open(test_file_path3) as f:
        IN22_src_3 = f.readlines()
    with open(test_file_path4) as f:
        IN22_src_4 = f.readlines()
    with open(test_file_path5) as f:
    ...
    model.eval()
    for test_inputs_1,test_inputs_2, test_inputs_3, test_inputs_4, test_tgt in tqdm(zip(IN22_src_1,IN22_src_2,IN22_src_3, IN22_src_4, IN22_tgt)):
        ...
        test_inputs_3 = [test_inputs_3]
        test_inputs_3 = ip.preprocess_batch(test_inputs_3, src_lang=src_lang_3, tgt_lang=tgt_lang)
        #print(test_inputs_2)
        test_inputs_3 = indic_indic_tokenizer(
            test_inputs_3,
            src=True,
            truncation=True,
            padding="longest",
        ...
        test_inputs_4 = [test_inputs_4]
        test_inputs_4 = ip.preprocess_batch(test_inputs_4, src_lang=src_lang_4, tgt_lang=tgt_lang)
        #print(test_inputs_2)
        test_inputs_4 = indic_indic_tokenizer(
            test_inputs_4,
            src=True,
            truncation=True,
            padding="longest",
        ...
        test_shape3 = test_inputs_3.input_ids.shape[-1]
        test_shape4 = test_inputs_4.input_ids.shape[-1]
        inputs = {
            'input_ids': torch.cat((test_inputs_1.input_ids,test_inputs_2.input_ids,test_inputs_3.input_ids,test_inputs_4.input_ids),dim=1),
            'attention_mask': torch.cat((test_inputs_1.attention_mask,test_inputs_2.attention_mask,test_inputs_3.attention_mask,test_inputs_4.attention_mask),dim=1),
        }
        ...
        print('Output')
        test_outputs = test_outputs[:,test_shape1+test_shape2+test_shape3+test_shape4:]
        #print(test_outputs)
