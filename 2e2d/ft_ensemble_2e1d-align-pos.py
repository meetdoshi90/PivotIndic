...
class DualEncoderDualDecoder(PreTrainedModel):
    ...
    def forward(self,
        ...
        if eval == False:
            ...
            labels = torch.ones(pooled_enc_1_states.shape[0]).to(pooled_enc_1_states.device)
            loss += torch.nn.functional.cosine_embedding_loss(pooled_enc_1_states,pooled_enc_2_states,labels,margin=0.1)
        else:
            loss = None
