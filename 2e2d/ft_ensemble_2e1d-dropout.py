...
class ReplaceTokensEarly(torch.nn.Module):
    def __init__(self,p=0.2, unk_id=3):
        super().__init__()
        self.p = p
        self.unk_id = unk_id

    def forward(self, x):
        # x is a token sequence of B, C
        # we will only sample tokens across C
        if self.training:
            # train() -> training logic
            mask = torch.distributions.Bernoulli(probs=(1-self.p)).sample(x.size())
            x[~mask.bool()] = self.unk_id
            return x
        else:
...
class ReplaceTokensLate(torch.nn.Module):
    def __init__(self,p=0.2,replace_type='random'):
        super().__init__()
        self.p = p
        self.replace_type = replace_type # 'random' or 'zeros'
        
    def forward(self, x):
        # x is a token sequence of B, C, V
        # we will only sample tokens across C
        if self.training:
            # train() -> training logic
            mask = torch.distributions.Bernoulli(probs=(1-self.p)).sample(x.size()[:-1])
            if self.replace_type=='random':
                init = torch.randn(x[~mask.bool()].shape).to(DEVICE)
            else:
                init = torch.zeros(x[~mask.bool()].shape).to(DEVICE)
            x[~mask.bool()] = init 
            return x
        else:
            # eval()  -> inference logic
...
class DualEncoderDualDecoder(PreTrainedModel):
    def __init__(self,model_names,alpha=0.5,beta=0.5,PAD_ID=1,replace='late',replace_type='random',replace_prob=0.2,config=None,genconfig=None):
        ...
        if self.replace=='early':
            self.replace_module = ReplaceTokensEarly(p=self.replace_prob)
        else:
            self.replace_module = ReplaceTokensLate(p=self.replace_prob,replace_type=self.replace_type)
    ...
    def forward(self,
        ...
        if self.replace=='early':
            inputs_1.input_ids = self.replace_module(inputs_1.input_ids)
            inputs_2.input_ids = self.replace_module(inputs_2.input_ids)
        ...
        if self.replace=='late':
            concatenated_hidden_states = self.replace_module(concatenated_hidden_states)
...
class ModelArguments:
    ...
    replace_prob: Optional[float] = field(
        default=0.2, metadata={"help": "Replace probability for tokens"}
    )

    replace: Optional[str] = field(
        default='late', metadata={"help": "Replace tokens early or late"}
    )

    replace_type: Optional[str] = field(
        default='random', metadata={"help": "If replace tokens late then random or zeros"}
    )
...
if __name__ == "__main__":
    ...
    model = DualEncoderDualDecoder(model_names,PAD_ID=PAD_ID, alpha=model_args.alpha, beta=model_args.beta, replace=model_args.replace, replace_type=model_args.replace_type, replace_prob=model_args.replace_prob, config=config, genconfig=genconfig).to(device)
