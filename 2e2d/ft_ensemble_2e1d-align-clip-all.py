...
class DualEncoderDualDecoder(PreTrainedModel):
    ...
    def forward(self,
        ...
        if eval == False:
            ...
            # enc align loss 
            pooled_enc_1_states = enc_out_1.last_hidden_state.sum(dim=1)
            pooled_enc_1_states = pooled_enc_1_states / torch.linalg.vector_norm(pooled_enc_1_states, dim=-1,keepdim=True)
            pooled_enc_2_states = enc_out_2.last_hidden_state.sum(dim=1)
            pooled_enc_2_states = pooled_enc_2_states / torch.linalg.vector_norm(pooled_enc_2_states, dim=-1,keepdim=True)
            pooled_dec_states = dec_out.last_hidden_state.sum(dim=1)
            pooled_dec_states = pooled_dec_states / torch.linalg.vector_norm(pooled_dec_states,dim=-1,keepdim=True)

            #align src1 and src2
            logits = torch.tensordot(pooled_enc_1_states,pooled_enc_2_states,dims=([1],[1]))
            labels = torch.arange(pooled_enc_1_states.shape[0]).to(logits.device)
            loss_src_1 = F.cross_entropy(logits,labels)
            loss_src_2 = F.cross_entropy(logits.T,labels)
            loss += (loss_src_1+loss_src_2)/(2.0*pooled_enc_1_states.shape[0])

            #align src1 and out
            logits = torch.tensordot(pooled_enc_1_states,pooled_dec_states,dims=([1],[1]))
            #labels = torch.arange(pooled_enc_1_states.shape[0]).to(logits.device)
            loss_src_1 = F.cross_entropy(logits,labels)
            loss_src_2 = F.cross_entropy(logits.T,labels)
            loss += (loss_src_1+loss_src_2)/(2.0*pooled_enc_1_states.shape[0])
            ...
            #align src2 and out
            logits = torch.tensordot(pooled_enc_2_states,pooled_dec_states,dims=([1],[1]))
            #labels = torch.arange(pooled_enc_2_states.shape[0]).to(logits.device)
            loss_src_1 = F.cross_entropy(logits,labels)
            loss_src_2 = F.cross_entropy(logits.T,labels)
            loss += (loss_src_1+loss_src_2)/(2.0*pooled_enc_2_states.shape[0])
